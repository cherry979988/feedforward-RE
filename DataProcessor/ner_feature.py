        ...
        for mention in mentions:
            try:
                if isEntityMention:
                    ...
                    # assert label1 != None and label2 != None
                    m_id = '%s_%s'%(label1, label2)
        ...
        for mention in mentions:
            if not isEntityMention:
                if sentence.questionPositions:
                    questionPositions = ' '.join([str(p) for p in sentence.questionPositions])
                    question_file.write(str(mention_count)+'\t'+question+'\t'+questionPositions+'\n')
                else:
                    question_file.write(str(mention_count)+'\t'+question+'\n')
            try:
                if isEntityMention:
                    m_id = '%s_%d_%d'%(sentence.sentId, mention.start, mention.end)
                else:
                    # m_id = '%d_%d_%d_%d_%d'%(sentence.sentId, mention.em1Start, mention.em1End, mention.em2Start, mention.em2End)
                    label1 = sentence.get_em_text(mention.em1Start, mention.em1End)
                    label2 = sentence.get_em_text(mention.em2Start, mention.em2End)
                    assert label1 != None and label2 != None
                    m_id = '%s_%s'%(label1, label2)
...
def filter(featurefile, trainfile, featureout,trainout):
    ...
    for line in f:
        ...
        frequency = int(seg[2])
        if frequency>=2:
            if (seg[0] in featuremap):
                old2new[seg[1]] = featuremap[seg[0]][0]
                continue
