...
class NLPParser(object):
    ...
    """
    def __init__(self):
        self.parser = CoreNLPClient(default_annotators=['ssplit', 'tokenize', 'pos'], server="http://localhost:9001")
...
def parse(sentences, g, lock, procNum, isTrain, nOfNones, parsePOSBeforehand=False):
    rmCount = 0
    discardRmCount = 0
    parser = NLPParser()
    ...
    for line in sentences:
        ...
        count += 1
        if(len(buffered) == 5):
            rmCount, discardRmCount = process(buffered, parser, g, lock, isTrain, rmCount, discardRmCount, nOfNones)
            buffered = []
            sys.stdout.write("Process %d, parsed %d sentences, Time: %d sec\r" % (procNum, count, time.time() - start) )
            sys.stdout.flush()
    if(len(buffered) > 0):
        rmCount, discardRmCount = process(buffered, parser, g, lock, isTrain, rmCount, discardRmCount, nOfNones, posAndTokensMap)
    print procNum, rmCount, discardRmCount, '\n'


def process(buffered, parser, g, lock, isTrain, rmCount, discardRmCount, nOfNones, posAndTokensMap=None):
        ...
        try:
            ...
            if len(visitedEmPairs) > 0:
                for emPair in visitedEmPairs:
                    labels = list(visitedEmPairs[emPair])
                    if 'None' in labels:
                        labels.remove('None')
                        if len(labels) == 0:
                            if isTrain:
                                ...
                                #assert nOfNones == 5
                                if nOfNones > 1:
                                    for ni in range(nOfNones):
                                        labels.append('None_'+str(ni))
                                else:
                                    labels.append('None')
                                #print labels
                                #assert len(labels) == nOfNones
                            else:
                                labels.append('None')
                    relationMentions.append({'em1Start':emPair[0][0], 'em1End':emPair[0][1], 'em2Start':emPair[1][0], 'em2End':emPair[1][1], 'numOfEMBetween':numOfEMBetweenMap[emPair], 'labels':labels})
            newsent = dict()
