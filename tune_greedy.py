...
def get_best_output_dropout(dataset, input_dropout, bsize):
    fin = open('tune_log.pkl', 'rb')
    d = pickle.load(fin)
    best_val_f1 = 0
    best_output_dropout = 0
    for key in d:
        if key[0]==dataset and key[2]==input_dropout and key[3]==bsize:
            if d[key]>best_val_f1:
                best_val_f1 = d[key]
                best_output_dropout = key[1]
    return best_output_dropout

def get_best_input_dropout(dataset, output_dropout, bsize):
    fin = open('tune_log.pkl', 'rb')
    d = pickle.load(fin)
    best_val_f1 = 0
    best_input_dropout = 0
    for key in d:
        if key[0]==dataset and key[1]==output_dropout and key[3]==bsize:
            if d[key]>best_val_f1:
                best_val_f1 = d[key]
                best_input_dropout = key[2]
    return best_input_dropout

def get_best_bsize(dataset, output_dropout, input_dropout):
    fin = open('tune_log.pkl', 'rb')
    d = pickle.load(fin)
    best_val_f1 = 0
    best_bsize = 0
    for key in d:
        if key[0]==dataset and key[1]==output_dropout and key[2]==input_dropout:
            if d[key]>best_val_f1:
                best_val_f1 = d[key]
                best_bsize = key[3]
    return best_bsize
...
best_output_dropout = get_best_output_dropout(dataset, default_input_dropout, default_bsize)

for input_dropout in input_dropout_list:
    cmd1 = 'CUDA_VISIBLE_DEVICES=%s python3 FFNN/run.py %s %s %s %s %d'\
        % (devices, dataset, best_output_dropout, input_dropout, default_bsize, tune_time_seed)
    print(cmd1)
    subprocess.call(cmd1,shell=True)

best_input_dropout = get_best_input_dropout(dataset, best_output_dropout, default_bsize)

# for bsize in bsize_list:
#     cmd1 = 'CUDA_VISIBLE_DEVICES=%s python3 FFNN/run.py %s %s %s %s %d'\
#         % (devices, dataset, best_output_dropout, best_input_dropout, bsize, tune_time_seed)
#     print(cmd1)
#     subprocess.call(cmd1,shell=True)    \
...
for i in [1,2,3,4,5]:
    cmd1 = 'CUDA_VISIBLE_DEVICES=%s python3 FFNN/run.py %s %s %s %s %d'\
        % (devices, dataset, best_output_dropout, best_input_dropout, best_bsize, i)
    print(cmd1)
    subprocess.call(cmd1,shell=True)
